...
def hybrid_differential_evolution(func, bounds, args=(), strategy='best1bin',
    ...
    g_hybrid : int, optional
        Si se especifica, define la cantidad de generaciones tras las cuales se 
        interrumpe la evolución estándar para aplicar una optimización local (SLSQP)
        al mejor individuo actual de la población.
    ...
    def solve(self):
        ...
        for nit in range(1, self.maxiter + 1):
            ...
            if self.g_hybrid is not None and nit % self.g_hybrid == 0:
                if self.disp:
                    print(f"HDE: Ejecutando optimización local SLSQP en generación {nit}...")
                
                # Optimización local con SLSQP en el mejor individuo actual
                res_local = minimize(
                    self.func,
                    self.x,
                    method='SLSQP',
                ...
                # Reemplazar al individuo original en la población (siempre que mejore o mantenga viabilidad optimizada)
